...
cell::cell(const vector<natmax>& upper_bound) noexcept
{
...
cell::cell(vector<natmax>&& upper_bound) noexcept
{
	span<natmax> upper_bound_span(const_cast<natmax*>(upper_bound.data()), find_if(upper_bound.rbegin(), upper_bound.rend(), [](natmax x) { return x != 0; }).base() - upper_bound.begin());
	content.resize(upper_bound_span.size() * 2, 0);
	memcpy(content.data() + upper_bound_span.size(), upper_bound_span.data(), upper_bound_span.size() * sizeof(natmax));
...
span<natmax> cell::value() const noexcept
{
	if (content.empty()) {
		return span<natmax>{};
	}
...
span<natmax> cell::number_of_states() const noexcept
{
	if (content.empty()) {
		return span<natmax>{};
	}
...
cell::cell(const cell& right) noexcept
{
...
void cell::operator=(span<natmax> value) noexcept
{
...
void cell::operator=(const cell& right) noexcept
{
	content = right.content;
}

void cell::operator=(cell&& right) noexcept
{
	content = right.content;
}
...
void cell::limit_right_value_then_increase(cell& left, const cell& right) noexcept
{
	...
	cell limited_right(right.number_of_states(), span<natmax>(remainder.data(), remainder.size()));
	// limited_right = cell(right.number_of_states(), remainder)
...
// 前置条件 P: left, right 皆为有效的数胞 (not left.content.empty() and not right.content.empty())
// 后置条件 Q: left <- left - right
void cell::limit_right_value_then_decrease(cell& left, const cell& right) const noexcept
{
	// 前置条件: not left.content.empty() and not right.content.empty()
...
// 前置条件 P: left, right 皆为有效的数胞 (not left.content.empty() and not right.content.empty())
// 后置条件 Q: left <- left * right
void cell::limit_right_value_then_multiply(cell& left, const cell& right) noexcept
{
	// 前置条件: not left.content.empty() and not right.content.empty()
...
cell cell::operator+(const cell& right) const noexcept
{
	...
	cell result = *this;
	result += right;
	runtime_assert(natural_number(result.value()) == (natural_number(this->value()) + natural_number(right.value())) % natural_number(number_of_states()), "数胞的+函数的后置条件不被满足，自身的状态数为" + generate_information_of_linear_table(number_of_states()) + "，自身的值为" + generate_information_of_linear_table(this->value()) + "，操作数的值为" + generate_information_of_linear_table(right.value()));
	return std::move(result);
}
...
cell cell::operator-(const cell& right) const noexcept
{
	...
	result -= right;
	integer_number U = power(natural_to_integer(vector<natmax>{ 0, 1 }), number_of_states().size());
	runtime_assert(natural_to_integer(result.value()) == (natural_to_integer(this->value()) - natural_to_integer(right.value()) + U) % natural_to_integer(number_of_states()), "数胞的-函数的后置条件不被满足，自身的状态数为" + generate_information_of_linear_table(number_of_states()) + "，自身的值为" + generate_information_of_linear_table(this->value()) + "，操作数的值为" + generate_information_of_linear_table(right.value()));
	return std::move(result);
}
...
cell cell::operator/(const cell& right) const
{
	...
	cell result = *this;
	result /= right;
	runtime_assert(natural_number(result.value()) == natural_number(this->value()) / natural_number(right.value()), "数胞的/函数的后置条件不被满足，自身的状态数为" + generate_information_of_linear_table(number_of_states()) + "，自身的值为" + generate_information_of_linear_table(this->value()) + "，操作数的值为" + generate_information_of_linear_table(right.value()));
	return std::move(result);
}
...
// 前置条件 P: *this, right 皆有效 (not this->content.empty() and not right.content.empty())
// 后置条件 Q: 输出 *this == right
bool cell::operator==(const cell& right) const noexcept
{
	span<natmax> current_value = this->value();
	span<natmax> right_value = right.value();
...
// 前置条件 P: *this, right 皆有效 (not this->content.empty() and not right.content.empty())
// 后置条件 Q: 输出 *this <=> right
weak_ordering cell::operator<=>(const cell& right) const noexcept
{
	span<natmax> current_value = this->value();
	span<natmax> right_value = right.value();
	runtime_assert(not current_value.empty() and not right_value.empty(), "数胞的<=>函数的前置条件不被满足");
