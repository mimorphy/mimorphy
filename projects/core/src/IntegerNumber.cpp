...
bool integer_number::bad() const noexcept
{
	...
	if (this->content.back() >> (sizeof(natmax) * WORD_SIZE - 1) == 1 and this->content.size() > 1) {
		return this->content[this->content.size() - 2] >> (sizeof(natmax) * WORD_SIZE - 1) == 1;
	}
	else if (this->content.back() >> (sizeof(natmax) * WORD_SIZE - 1) == 0 and this->content.size() > 1) {
...
void integer_number::operator=(span<natmax> value) noexcept
{
	...
	else if (value.back() >> (sizeof(natmax) * WORD_SIZE - 1) == 1 and value.size() > 1) {
...
void integer_number::operator+=(const integer_number& right) noexcept
{
	if (content.empty() or right.content.empty()) {
		return;
	}
	...
	// 处理符号相同时计算导致符号变号的情况
	if (not sign_is_different) {
		if (right.content.back() >> (sizeof(natmax) * WORD_SIZE - 1) == 0 and content.back() >> (sizeof(natmax) * WORD_SIZE - 1) == 1) {
			content.push_back(0);
		}
		else if (right.content.back() >> (sizeof(natmax) * WORD_SIZE - 1) == 1 and content.back() >> (sizeof(natmax) * WORD_SIZE - 1) == 0) {
...
void integer_number::operator-=(const integer_number& right) noexcept
{
	...
	if (sign_is_different) {
		if (content.back() >> (sizeof(natmax) * WORD_SIZE - 1) == 1 and right.content.back() >> (sizeof(natmax) * WORD_SIZE - 1) == 1) {
			content.push_back(0);
		}
		else if (content.back() >> (sizeof(natmax) * WORD_SIZE - 1) == 0 and right.content.back() >> (sizeof(natmax) * WORD_SIZE - 1) == 0) {
			content.push_back(natmax_max);
		}
	}
	clear_invalid_bits(content);
...
void integer_number::operator*=(const integer_number& right) noexcept
{
	...
	if (not sign_is_different and content.back() >> (sizeof(natmax) * WORD_SIZE - 1) == 1) {
		content.push_back(0);
	}
	else if (sign_is_different and content.back() >> (sizeof(natmax) * WORD_SIZE - 1) == 0) {
		content.push_back(natmax_max);
	}
	clear_invalid_bits(content);
}
...
void integer_number::operator%=(const integer_number& right)
{
	...
	if (sign_is_negative and not (content.size() == 1 and content.back() == 0)) {
		*this -= right;
	}
}
...
bool integer_number::operator<(const integer_number& right) const noexcept
{
	if (content.back() >> (sizeof(natmax) * WORD_SIZE - 1) == 0 and right.content.back() >> (sizeof(natmax) * WORD_SIZE - 1) == 1) {
		return false;
	}
	else if (content.back() >> (sizeof(natmax) * WORD_SIZE - 1) == 1 and right.content.back() >> (sizeof(natmax) * WORD_SIZE - 1) == 0) {
		return true;
	}
	sizevalue min_size = min(content.size(), right.content.size());
	if (content.size() < right.content.size()) {
