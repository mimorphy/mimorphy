...
cell_view::cell_view(const vector<natmax>& upper_bound) noexcept
{
	_number_of_states_ = span<natmax>(const_cast<natmax*>(upper_bound.data()), find_if(upper_bound.rbegin(), upper_bound.rend(), [](natmax x) { return x != 0; }).base() - upper_bound.begin());
}

cell_view::cell_view(vector<natmax>&& upper_bound) noexcept
{
	_number_of_states_ = span<natmax>(const_cast<natmax*>(upper_bound.data()), find_if(upper_bound.rbegin(), upper_bound.rend(), [](natmax x) { return x != 0; }).base() - upper_bound.begin());
}

span<natmax> cell_view::value() const noexcept
{
	return _value_;
}

span<natmax> cell_view::number_of_states() const noexcept
{
	return _number_of_states_;
}
...
bool cell_view::bad() const noexcept
{
...
cell_view::cell_view(const cell_view& right) noexcept
{
	_number_of_states_ = right.number_of_states();
	_value_ = right.value();
}

cell_view::cell_view(cell_view&& right) noexcept
{
	_number_of_states_ = right.number_of_states();
	_value_ = right.value();
}

void cell_view::operator=(span<natmax> value) noexcept
{
...
void cell_view::operator=(const cell_view& right) noexcept
{
	_value_ = right.value();
    _number_of_states_ = right.number_of_states();
}

void cell_view::operator=(cell_view&& right) noexcept
{
	_value_ = right.value();
    _number_of_states_ = right.number_of_states();
}
...
static void decrease_between_bit_spans(bit_span& minuend, const bit_span& subtrahend) noexcept
{
	...
	for (sizevalue i = 0; i < culculate_length; ++i) {
		natural_number two_power_i = vector<natmax>{ 1 };
		for (sizevalue j = 0; j < i; ++j) {
			two_power_i *= natural_number({ 2 });
		}
		runtime_assert(natural_number(bit_span_to_number(original_minuend.subspan(0, i))) - natural_number(bit_span_to_number(subtrahend.subspan(0, i))) - two_power_i * natural_number({ borrow ? 1ULL : 0ULL }) == natural_number(bit_span_to_number(minuend.subspan(0, i))), "decrease_between_bit_spans 的循环不变式与循环不符");
		if (minuend[i] == true) {
	...
	natural_number two_power_culculate_length = vector<natmax>{ 1 };
	for (sizevalue j = 0; j < culculate_length; ++j) {
		two_power_culculate_length *= natural_number({ 2 });
	}
	runtime_assert(natural_number(bit_span_to_number(original_minuend.subspan(0, culculate_length))) - natural_number(bit_span_to_number(subtrahend.subspan(0, culculate_length))) - two_power_culculate_length * natural_number({ borrow ? 1ULL : 0ULL }) == natural_number(bit_span_to_number(minuend.subspan(0, culculate_length))), "decrease_between_bit_spans 的循环不变式与循环不符");
	...
	if (borrow) {
		...
		do {
			runtime_assert(natural_number(bit_span_to_number(minuend.subspan(0, culculate_length))) + two_power_culculate_length * natural_number({ borrow ? 1ULL : 0ULL }) == natural_number(bit_span_to_number(original_minuend.subspan(0, culculate_length))) - natural_number(bit_span_to_number(subtrahend.subspan(0, culculate_length))), "decrease_between_bit_spans 在借位传播中的循环不变式与循环不符");
			// 翻转当前高位位 (借位传播)
...
static natural_number power(natural_number& base, sizevalue exponential)
{
	...
	for (sizevalue i = 0; i < exponential; ++i) {
		result *= base;
	}
	return std::move(result);
}
...
// 霍尔逻辑规范：
// 前置条件 P: left, right 皆为有效的数胞 (not left.value().empty() and not right.value().empty())
// 后置条件 Q: left <- left + right
void cell_view::limit_right_value_then_increase(cell_view& left, const cell_view& right) noexcept
{
	// 前置条件: not left.value().empty() and not right.value().empty()
	runtime_assert(not left.value().empty() and not right.value().empty(), "limit_right_value_then_increase 的前置条件不被满足");
	...
	runtime_assert(natural_number(remainder) == natural_number(copy_right_value) % left.number_of_states(), "limit_right_value_then_increase 中的 mod_value_by_value 的结果不符合预期");
	cell_view limited_right(right.number_of_states(), span<natmax>(remainder.data(), remainder.size()));
	// limited_right = cell_view(right.number_of_states(), remainder)
	natural_number old_left = left.value();
	left += limited_right;
	runtime_assert(left.value() == (old_left + limited_right.value()) % natural_number(this->number_of_states()), "limit_right_value_then_increase 的后置条件不被满足");
...
static void propagate_carry_in_increase(span<natmax>&& parts, nat8& carry) noexcept
{
	...
	vector<natmax> old_parts(parts.begin(), parts.end());
	nat8 old_carry = carry;
	nat8 original_carry = carry;
	for (sizevalue i = 0; i < parts.size(); ++i) {
		natural_number left_k{};
		natural_number right_k{};
		natural_number B_power_k({ 1 });
		for (sizevalue k = 0; k < i; ++k) {
			left_k += natural_number({ parts[k] }) * natural_number(B_power_k);
			right_k += natural_number({ old_parts[k] }) * natural_number(B_power_k);
			B_power_k *= natural_number({ 0, 1 });
		}
		old_carry = carry;
		...
		runtime_assert(left_k == right_k + natural_number({ old_carry }) - natural_number({ carry }) * B_power_k, "propagate_carry_in_increase 的循环不变式与实际执行结果不符");
	}
	natural_number B_power_size = power(natural_number({ 0, 1 }), parts.size());
	runtime_assert(natural_number(parts) == natural_number(old_parts) + natural_number({ old_carry }) - natural_number({ carry }) * B_power_size, "propagate_carry_in_increase 的循环不变式与实际执行结果不符");
	
	natural_number U = power(natural_number({ 0, 1 }), parts.size());
	runtime_assert((natural_number(parts) == natural_number(old_parts) + natural_number({ original_carry }) and carry == 0) or (natural_number(parts) == natural_number(old_parts) + natural_number({ original_carry }) - U and carry == 1), "propagate_carry_in_increase 的后置条件不被满足");
...
static void limit_value_after_increase(span<natmax>&& number_of_states, span<natmax>&& value) noexcept
{
	...
	if (number_of_states.size() == 1) {
		integer_number old_value(value);
		integer_number U = power(natural_to_integer(vector<natmax>{ 0, 1 }), number_of_states.size());
		value.front() -= (number_of_states.front());
		if (natural_to_integer(value) >= natural_to_integer(number_of_states)) {
			runtime_assert(natural_to_integer(value) == old_value - natural_to_integer(number_of_states), "limit_value_after_increase 的后置条件不被满足");
		}
		else {
			runtime_assert(natural_to_integer(value) == old_value - (U - natural_to_integer(number_of_states)), "limit_value_after_increase 的后置条件不被满足");
		}
		return;
	}
	...
	for (auto it = value.begin(); it != value.end(); ++it) {
		...
		integer_number B_power_i = power(natural_to_integer(vector<natmax>{ 0, 1 }), i);
		runtime_assert(natural_to_integer(value) == old_value - natural_to_integer(number_of_states.subspan(0, i)) + natural_to_integer({ borrow }) * B_power_i and i <= value.size(), "limit_value_after_increase 的循环不变式与其执行结果不符");
	...
	integer_number B_power_size = power(natural_to_integer(vector<natmax>{ 0, 1 }), value.size());
	runtime_assert(natural_to_integer(value) == old_value - natural_to_integer(number_of_states) + natural_to_integer({ borrow }) * B_power_size, "limit_value_after_increase 的循环不变式与其执行结果不符");

	if (old_value >= natural_to_integer(number_of_states)) {
		runtime_assert(natural_to_integer(value) == old_value - natural_to_integer(number_of_states), "limit_value_after_increase 的后置条件不被满足");
	}
	else {
		runtime_assert(natural_to_integer(value) == old_value + (U - natural_to_integer(number_of_states)), "limit_value_after_increase 的后置条件不被满足");
...
// 霍尔逻辑规范：
// 前置条件 P: *this, right 皆有效 (not this->value().empty() and not right.value().empty())
// 后置条件 Q: *this <- *this + right
void cell_view::operator+=(const cell_view& right) noexcept
{
	// 前置条件: not this->value().empty() and not right.value().empty()
	runtime_assert(not this->value().empty() and not right.value().empty(), "数胞的+=函数的前置条件不被满足");
	...
	//   其中 Low_k(X) 表示 X 的低 k 位组成的自然数
	natural_number old_self(this->value());
	...
	for (auto it = current_value.begin(); it != current_value.end(); ++it) {
		...
		natural_number B_power_i = power(natural_number({ 0, 1 }), i);
		runtime_assert(this->value() == old_self + right_value.subspan(0, i) - natural_number({ carry }) * B_power_i and i <= current_value.size(), "数胞的+=函数的循环不变式与其执行结果不符，自身的状态数为" + generate_information_of_linear_table(number_of_states()) + "，自身的值为" + generate_information_of_linear_table(old_self.content) + "，操作数的值为" + generate_information_of_linear_table(right_value));
	...
	natural_number B_power_size = power(natural_number({ 0, 1 }), current_value.size());
	runtime_assert(this->value() == old_self + right_value - natural_number({carry}) * B_power_size, "数胞的+=函数的循环不变式与其执行结果不符，自身的状态数为" + generate_information_of_linear_table(number_of_states()) + "，自身的值为" + generate_information_of_linear_table(old_self.content) + "，操作数的值为" + generate_information_of_linear_table(right_value));
	// 计算完成后有两种情况
	...
	runtime_assert(this->value() == (old_self + right.value()) % natural_number(this->number_of_states()), "数胞的+=函数的后置条件不被满足，自身的状态数为" + generate_information_of_linear_table(number_of_states()) + "，自身的值为" + generate_information_of_linear_table(old_self.content) + "，操作数的值为" + generate_information_of_linear_table(right_value));
...
// 前置条件 P: left, right 皆为有效的数胞 (not left.value().empty() and not right.value().empty())
// 后置条件 Q: left <- left - right
void cell_view::limit_right_value_then_decrease(cell_view& left, const cell_view& right) const noexcept
{
	// 前置条件: not left.value().empty() and not right.value().empty()
	runtime_assert(not left.value().empty() and not right.value().empty(), "limit_right_value_then_decrease 的前置条件不被满足");
	vector<natmax> copy_right_value(right.value().begin(), right.value().end());
	// copy_right_value = right.value()
	vector<natmax> remainder = mod_value_by_value(span<natmax>(copy_right_value.data(), copy_right_value.size()), left.number_of_states());
	// remainder = copy_right_value mod left.number_of_states() = right.value() mod left.number_of_states()
	runtime_assert(natural_to_integer(remainder) == natural_to_integer(copy_right_value) % natural_to_integer(left.number_of_states()), "limit_right_value_then_decrease 中的 mod_value_by_value 的执行结果与预期不符");
	cell_view limited_right(right.number_of_states(), span<natmax>(remainder.data(), remainder.size()));
	// limited_right = cell_view(right.number_of_states(), remainder)
	integer_number old_left = natural_to_integer(left.value());
	left -= limited_right;
	integer_number U = power(integer_number({ 0, 1 }), left.number_of_states().size());
	runtime_assert(natural_to_integer(left.value()) == (old_left - natural_to_integer(limited_right.value()) + U) % natural_to_integer(this->number_of_states()), "limit_right_value_then_decrease 的后置条件不被满足");
	// 后置条件: left <- left - limited_right = left - right
}
...
static void propagate_borrow_in_decrease(span<natmax>&& parts, nat8& borrow) noexcept
{
	...
	vector<natmax> old_parts(parts.begin(), parts.end());
	nat8 old_borrow = borrow;
	nat8 original_borrow = borrow;
	for (sizevalue i = 0; i < parts.size(); ++i) {
		integer_number left_k{};
		integer_number right_k{};
		integer_number B_power_k({ 1 });
		for (sizevalue k = 0; k < i; ++k) {
			left_k += natural_to_integer({ parts[k] }) * integer_number(B_power_k);
			right_k += natural_to_integer({ old_parts[k] }) * integer_number(B_power_k);
			B_power_k *= integer_number({ 0, 1 });
		}
		...
		runtime_assert(left_k == right_k - integer_number({ old_borrow }) + integer_number({ borrow }) * B_power_k, "propagate_carry_in_increase 的循环不变式与实际执行结果不符");
	}
	integer_number B_power_size = power(integer_number({ 0, 1 }), parts.size());
	runtime_assert(natural_to_integer(parts) == natural_to_integer(old_parts) - integer_number({ old_borrow }) + integer_number({ borrow }) * B_power_size, "propagate_carry_in_increase 的循环不变式与实际执行结果不符");

	integer_number U = power(integer_number({ 0, 1 }), parts.size());
	runtime_assert((natural_to_integer(parts) == natural_to_integer(old_parts) - integer_number({ original_borrow }) and borrow == 0) or (natural_to_integer(parts) == natural_to_integer(old_parts) - integer_number({ original_borrow }) + U and borrow == 1), "propagate_carry_in_increase 的后置条件不被满足");
...
static void limit_value_after_decrease(span<natmax>&& number_of_states, span<natmax>&& value) noexcept
{
	...
	// 当 number_of_states 的大小为1时，说明 value 的大小理应也为1，此时只需一次计算即可
	if (number_of_states.size() == 1) {
		integer_number old_value = natural_to_integer(value);
		integer_number U = power(natural_to_integer(vector<natmax>{ 0, 1 }), number_of_states.size());
		value.front() += (number_of_states.front());
		if (natural_to_integer(value) >= natural_to_integer(number_of_states)) {
			runtime_assert(natural_to_integer(value) == old_value - natural_to_integer(number_of_states), "limit_value_after_decrease 的后置条件不被满足");
		}
		else {
			runtime_assert(natural_to_integer(value) == old_value - (U - natural_to_integer(number_of_states)), "limit_value_after_decrease 的后置条件不被满足");
		}
		return;
	}
	// 如果value < number_of_states: value <- value + number_of_states，返回
	...
	for (auto it = value.begin(); it != value.end(); ++it) {
		sizevalue i = it - value.begin();
		integer_number B_power_i = power(natural_to_integer(vector<natmax>{ 0, 1 }), i);
		runtime_assert(natural_to_integer(value) == old_value + natural_to_integer(number_of_states.subspan(0, i)) - natural_to_integer({ carry }) * B_power_i and i <= value.size(), "limit_value_after_decrease 的循环不变式与其执行结果不符");
	...
	integer_number B_power_size = power(natural_to_integer(vector<natmax>{ 0, 1 }), value.size());
	runtime_assert(natural_to_integer(value) == old_value + natural_to_integer(number_of_states) - natural_to_integer({ carry }) * B_power_size, "limit_value_after_decrease 的循环不变式与其执行结果不符");
...
void cell_view::operator-=(const cell_view& right) noexcept
{
	...
	//   其中 Low_k(X) 表示 X 的低 k 位组成的自然数
	integer_number old_self = natural_to_integer(this->value());
	auto iterator_of_right = right_value.begin();
	nat8 borrow = 0; // 借位
	for (auto it = current_value.begin(); it != current_value.end(); ++it) {
		sizevalue i = it - current_value.begin();
		integer_number B_power_i = power(integer_number({ 0, 1 }), i);
		runtime_assert(natural_to_integer(this->value()) == old_self - natural_to_integer(right_value.subspan(0, i)) + integer_number({ borrow }) * B_power_i and i <= current_value.size(), "数胞的-=函数的循环不变式与其执行结果不符，自身的状态数为" + generate_information_of_linear_table(number_of_states()) + "，自身的值为" + generate_information_of_linear_table(old_self.content) + "，操作数的值为" + generate_information_of_linear_table(right_value));
	...
	integer_number B_power_size = power(integer_number({ 0, 1 }), current_value.size());
	runtime_assert(natural_to_integer(this->value()) == old_self - natural_to_integer(right_value) + integer_number({ borrow }) * B_power_size, "数胞的-=函数的循环不变式与其执行结果不符，自身的状态数为" + generate_information_of_linear_table(number_of_states()) + "，自身的值为" + generate_information_of_linear_table(old_self.content) + "，操作数的值为" + generate_information_of_linear_table(right_value));
	// 计算完成后有两种情况
	...
	runtime_assert(natural_to_integer(this->value()) == (old_self - natural_to_integer(right.value()) + U) % natural_to_integer(this->number_of_states()), "数胞的-=函数的后置条件不被满足，自身的状态数为" + generate_information_of_linear_table(number_of_states()) + "，自身的值为" + generate_information_of_linear_table(old_self.content) + "，操作数的值为" + generate_information_of_linear_table(right_value));
	// 后置条件: *this <- *this - right
}
...
// 前置条件 P: left, right 皆为有效的数胞 (not left.value().empty() and not right.value().empty())
// 后置条件 Q: left <- left * right
void cell_view::limit_right_value_then_multiply(cell_view& left, const cell_view& right) noexcept
{
	// 前置条件: not left.value().empty() and not right.value().empty()
	runtime_assert(not left.value().empty() and not right.value().empty(), "limit_right_value_then_multiply 的前置条件不被满足");
	vector<natmax> copy_right_value(right.value().begin(), right.value().end());
	// copy_right_value = right.value()
	vector<natmax> remainder = mod_value_by_value(span<natmax>(copy_right_value.data(), copy_right_value.size()), left.number_of_states());
	// remainder = copy_right_value mod left.number_of_states() = right.value() mod left.number_of_states()
	runtime_assert(natural_number(remainder) == natural_number(copy_right_value) % natural_number(left.number_of_states()), "limit_right_value_then_multiply 中的 mod_value_by_value 的执行结果不符合预期");
	cell_view limited_right(right.number_of_states(), span<natmax>(remainder.data(), remainder.size()));
	// limited_right = cell_view(right.number_of_states(), remainder)
	natural_number old_left = left.value();
	left *= limited_right;
	runtime_assert(natural_number(left.value()) == (old_left * right.value()) % left.number_of_states(), "limit_right_value_then_multiply 的后置条件不被满足");
	// 后置条件: left <- left * limited_right = left * right
}
...
static void propagate_carry_in_multiply(span<nathalf>&& parts, nathalf& carry) noexcept
{
	...
	// 循环不变式：
	//   ∑_{k=0}^{i-1}(parts[k] * B^k) = ∑_{k=0}^{i-1}(parts_{old}[k] * B^k) + carry_{old} - carry_{current} * B^i
	vector<natmax> old_parts(parts.size() / 2 + parts.size() % 2);
	memcpy(old_parts.data(), parts.data(), old_parts.size() * sizeof(natmax));
	nathalf old_carry = carry;
	nathalf original_carry = carry;
	for (sizevalue i = 0; i < parts.size(); ++i) {
		natural_number left_k{};
		natural_number right_k{};
		natural_number B_power_k({ 1 });
		for (sizevalue k = 0; k < i; ++k) {
			left_k += natural_number({ parts[k] }) * natural_number(B_power_k);
			right_k += natural_number({ old_parts[k] }) * natural_number(B_power_k);
			B_power_k *= natural_number({ NUMBER_OF_NATHALF_STATE });
		}
		old_carry = carry;
		...
		runtime_assert(left_k == right_k + natural_number({ old_carry }) - natural_number({ carry }) * B_power_k, "propagate_carry_in_multiply 的循环不变式与实际执行结果不符");
	}
	natural_number B_power_size = power(natural_number({ NUMBER_OF_NATHALF_STATE }), parts.size());
	runtime_assert(natural_number(span<natmax>((natmax*)parts.data(), parts.size() / 2 + parts.size() % 2)) == natural_number(old_parts) + natural_number({ old_carry }) - natural_number({ carry }) * B_power_size, "propagate_carry_in_increase 的循环不变式与实际执行结果不符");

	natural_number U = power(natural_number({ NUMBER_OF_NATHALF_STATE }), parts.size());
	runtime_assert(natural_number(span<natmax>((natmax*)parts.data(), parts.size() / 2 + parts.size() % 2)) == natural_number(old_parts) + natural_number({ original_carry }) and carry == 0, "propagate_carry_in_increase 的后置条件不被满足");
...
void cell_view::operator*=(const cell_view& right) noexcept
{
	...
	if (current_value.size() == 1 and right_value.size() == 1 and current_value.front() <= nat32_max and right_value.front() <= nat32_max) {
		...
		runtime_assert(natural_number(this->value()) == (old_self * right.value()) % this->number_of_states(), "数胞的*=函数的后置条件不被满足，自身的状态数为" + generate_information_of_linear_table(number_of_states()) + "，自身的值为" + generate_information_of_linear_table(old_self.content) + "，操作数的值为" + generate_information_of_linear_table(right_value));
		return;
	}
	...
	runtime_assert(natural_number(span<natmax>((natmax*)half_form_current_value.data(), half_form_current_value.size() / 2 + half_form_current_value.size() % 2)) == current_value, "数胞的*=函数的视图转换与预期不符，自身的状态数为" + generate_information_of_linear_table(number_of_states()) + "，自身的值为" + generate_information_of_linear_table(old_self.content) + "，操作数的值为" + generate_information_of_linear_table(right_value));
	runtime_assert(natural_number(span<natmax>((natmax*)half_form_right_value.data(), half_form_right_value.size() / 2 + half_form_right_value.size() % 2)) == right_value, "数胞的*=函数的视图转换与预期不符，自身的状态数为" + generate_information_of_linear_table(number_of_states()) + "，自身的值为" + generate_information_of_linear_table(old_self.content) + "，操作数的值为" + generate_information_of_linear_table(right_value));
	sizevalue size_of_intermediate_data = half_form_current_value.size() + half_form_right_value.size();
	...
	for (sizevalue i = 0; i < half_form_right_value.size(); ++i) {
		natural_number sum{};
		for (sizevalue m = 0; m < i; ++m) {
			natural_number temporary_sum{};
			for (sizevalue n = 0; n < half_form_current_value.size(); ++n) {
				temporary_sum += natural_number({ half_form_right_value[m] }) * natural_number({ half_form_current_value[n] }) * power(natural_number({ NUMBER_OF_NATHALF_STATE }), n);
			}
			sum += temporary_sum * power(natural_number({ NUMBER_OF_NATHALF_STATE }), m);
		}
		runtime_assert(natural_number(span<natmax>((natmax*)intermediate_data.data(), intermediate_data.size() / 2)) == sum, "数胞的*=函数的外层循环不变式与执行结果不符，自身的状态数为" + generate_information_of_linear_table(number_of_states()) + "，自身的值为" + generate_information_of_linear_table(old_self.content) + "，操作数的值为" + generate_information_of_linear_table(right_value));
		...
		natural_number last_intermediate_data = span<natmax>((natmax*)intermediate_data.data(), intermediate_data.size() / 2);
		for (sizevalue j = 0; j < half_form_current_value.size(); ++j) {
			natural_number sum{};
			natural_number B_power_k({ 1 });
			for (sizevalue k = 0; k < j; ++k) {
				sum += natural_number({ right_digital }) * natural_number({ half_form_current_value[k] }) * B_power_k;
				B_power_k *= natural_number({ NUMBER_OF_NATHALF_STATE });
			}
			runtime_assert(natural_number(span<natmax>((natmax*)intermediate_data.data(), intermediate_data.size() / 2)) == last_intermediate_data + (sum - natural_number({ carry }) * B_power_k) * power(natural_number({ NUMBER_OF_NATHALF_STATE }), i), "数胞的*=函数的内层循环不变式与执行结果不符，自身的状态数为" + generate_information_of_linear_table(number_of_states()) + "，自身的值为" + generate_information_of_linear_table(old_self.content) + "，操作数的值为" + generate_information_of_linear_table(right_value));
		...
		sum = natural_number{};
		natural_number B_power_k({ 1 });
		for (sizevalue k = 0; k < half_form_current_value.size(); ++k) {
			sum += natural_number({ right_digital }) * natural_number({ half_form_current_value[k] }) * B_power_k;
			B_power_k *= natural_number({ NUMBER_OF_NATHALF_STATE });
		}
		runtime_assert(natural_number(span<natmax>((natmax*)intermediate_data.data(), intermediate_data.size() / 2)) == last_intermediate_data + (sum - natural_number({ carry }) * B_power_k) * power(natural_number({ NUMBER_OF_NATHALF_STATE }), i), "数胞的*=函数的内层循环不变式与执行结果不符，自身的状态数为" + generate_information_of_linear_table(number_of_states()) + "，自身的值为" + generate_information_of_linear_table(old_self.content) + "，操作数的值为" + generate_information_of_linear_table(right_value));
	...
	natural_number sum{};
	for (sizevalue m = 0; m < half_form_right_value.size(); ++m) {
		natural_number temporary_sum{};
		for (sizevalue n = 0; n < half_form_current_value.size(); ++n) {
			temporary_sum += natural_number({ half_form_right_value[m] }) * natural_number({ half_form_current_value[n] }) * power(natural_number({ NUMBER_OF_NATHALF_STATE }), n);
		}
...
// 霍尔逻辑规范：
// 前置条件 P: *this, right 皆有效 (not this->value().empty() and not right.value().empty())，且 right 的值不为 0
// 后置条件 Q: *this <- *this / right
void cell_view::operator/=(const cell_view& right)
{
	// 前置条件: not this->value().empty() and not right.value().empty()
	runtime_assert(not this->value().empty() and not right.value().empty(), "数胞的/=函数的前置条件不被满足");
	...
	natural_number old_self = current_value;
	runtime_assert(not (right_value.size() == 1 and right_value.front() == 0), "数胞的/=函数中除数为0，自身的状态数为" + generate_information_of_linear_table(number_of_states()) + "，自身的值为" + generate_information_of_linear_table(old_self.content) + "，操作数的值为" + generate_information_of_linear_table(right_value));
	// 如果满足以下条件，可以直接进行单次相除得到结果
	if (current_value.size() == 1 and right_value.size() == 1) {
		current_value.front() /= right_value.front();
		runtime_assert(this->value() == old_self / natural_number(right_value), "数胞的/=函数的后置条件不被满足，自身的状态数为" + generate_information_of_linear_table(number_of_states()) + "，自身的值为" + generate_information_of_linear_table(old_self.content) + "，操作数的值为" + generate_information_of_linear_table(right_value));
		return;
	}
	// 如果被除数小于除数，则结果一定为 0
...
bool cell_view::operator==(const cell_view& right) const noexcept
{
	span<natmax> current_value = this->value();
	span<natmax> right_value = right.value();
...
// 前置条件 P: *this, right 皆有效 (not this->value().empty() and not right.value().empty())
// 后置条件 Q: 输出 *this <=> right
weak_ordering cell_view::operator<=>(const cell_view& right) const noexcept
{
	span<natmax> current_value = this->value();
	span<natmax> right_value = right.value();
	runtime_assert(not current_value.empty() and not right_value.empty(), "数胞的<=>函数的前置条件不被满足");
