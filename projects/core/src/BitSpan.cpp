...
bit_span::bit_span(pointer data, size_type bit_offset, size_type size_in_bits)
    ...
    if (bit_offset_ >= 8) {
        throw std::out_of_range("bit_offset must be in range [0, 7]");
    }
}
...
// bit_reference 实现
bit_reference::bit_reference(unsigned char* byte_ptr, size_t bit_pos) noexcept
: byte_ptr_(byte_ptr), bit_pos_(bit_pos) {}
