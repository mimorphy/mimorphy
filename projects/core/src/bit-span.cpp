...
bit_span::bit_span(pointer data, size_type bit_offset, size_type size_in_bits) : data_(data), bit_offset_(bit_offset), size_in_bits_(size_in_bits)
{
    if (bit_offset_ >= 8) {
...
bit_span::bit_span(iterator first, iterator last)
{
    ...
    if (last.byte_ptr() < data_) {
        throw std::invalid_argument("无效的迭代器范围：结束位置在开始位置之前");
    }
    ...
    if (size_in_bits_ > max_bits) {
        throw std::out_of_range("迭代器范围超出有效位范围");
    }
}
...
bit_span::reference bit_span::at(size_type idx)
{
    if (idx >= size_in_bits_){
        throw std::out_of_range("位视图索引超出范围");
    }
    return (*this)[idx];
}
...
{
    if (n > size_in_bits_) {
        throw std::out_of_range("bit_span::first：n 超出大小限制");
    }
...
{
    if (n > size_in_bits_) {
        throw std::out_of_range("bit_span::last：n 超出大小限制");
    }
    // 计算新起始位置
...
// bit_iterator 实现
bit_iterator::bit_iterator(unsigned char* ptr, size_t bit_pos) noexcept : byte_ptr_(ptr), bit_pos_(bit_pos) {}
...
const_bit_iterator::const_bit_iterator(const unsigned char* ptr, size_t bit_pos) noexcept : byte_ptr_(ptr), bit_pos_(bit_pos) {}

// 从可变迭代器构造
const_bit_iterator::const_bit_iterator(const bit_iterator& other) noexcept : byte_ptr_(other.byte_ptr_), bit_pos_(other.bit_pos_) {}
