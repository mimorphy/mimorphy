...
static void connect_predecessors_and_successors(regular_table& regular_table, vector<vector<state_transition>>& virtual_transitions_table, vector<sizevalue>& index_of_predecessors, vector<sizevalue> index_of_successors);
...
static void connect_predecessors_and_successors(regular_table& regular_table, vector<vector<state_transition>>& virtual_transitions_table, vector<sizevalue>& index_of_predecessors, vector<sizevalue> index_of_successors)
{
    for (auto index_of_predecessor : index_of_predecessors) {
        for (auto index_of_successor : index_of_successors) {
            while (regular_table[index_of_predecessor].type == state_transition_type::NEGATION_INTERSECTION && index_of_predecessor + 1 < regular_table.size()) {
