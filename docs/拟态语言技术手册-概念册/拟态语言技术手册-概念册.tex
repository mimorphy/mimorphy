...
\qquad 对于字符串$R_1 = \varepsilon, R_2$, $R_1 \cdot R_2 = R_2$；$R_2 \cdot R_1 = R_2$。
...
\qquad 对于非空字符串$S$，若有$R_0 = (S)$，则$q_{start}^{R_0} = q_{start}^{S}$，且$q_{end}^{R_0} = q_{end}^{S}$。

\qquad 对于非空字符串$P,S$，若有$R_0 = P \cdot S$，则$\forall q_s \in q_{start}^{S}, \forall q_e \in q_{end}^{P}, \exists \delta(q_e, c(q_s)) = q_s$。
...
状态定义：
\begin{itemize}
    \item 对于表达式$R = R_1 | R_2 | \cdots | R_n$，且$R_1, R_2, \cdots, R_n$均不为空字符串，有$q_{start}^{R} = \{q_{start}^{R_1},q_{start}^{R_2},\cdots,q_{start}^{R_n}\}$
    \item 对于表达式$R = R_1 | R_2 | \cdots | R_n$，且$R_1, R_2, \cdots, R_n$均不为空字符串，有$q_{end}^{R} = \{q_{end}^{R_1},q_{end}^{R_2},\cdots,q_{end}^{R_n}\}$
