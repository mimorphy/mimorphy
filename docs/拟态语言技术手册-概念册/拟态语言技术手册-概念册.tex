...
\qquad 字符是字母表中的元素，字符串是由字符组成的有限序列，空字符串记为$\varepsilon$，字母表中的任意字符记为$\bullet$。
...
\qquad 首先，对正则表达式$R$中的每个字面字符和操作符出现进行标记，使每个出现具有唯一的位置索引。
...
\begin{itemize}
    ...
    \item 唯一位置索引$i(p)$（默认从左到右按顺序编号，特殊字符也需要占用编号）。
\end{itemize}

例如，对于正则表达式 $R = \text{"aa*b"}$，字面字符出现为：位置1的'a'、位置2的'a'、位置3的'*'、位置4的'b'。
因此，$P = \{p_1, p_2, p_3\}$，其中$c(p_1) = 'a', c(p_2) = 'a', c(p_3) = 'b', i(p_1) = 1, i(p_2) = 2, i(p_3) = 4$。
...
\qquad 对于表达式$R_1 = \varepsilon$和任意的$R_2$, $R_1 \cdot R_2 = R_2$；$R_2 \cdot R_1 = R_2$。
...
\qquad 对于非空表达式$S$，若有$R_0 = (S)$，则$q_{start}^{R_0} = q_{start}^{S}$，且$q_{end}^{R_0} = q_{end}^{S}$。
...
状态定义：
\begin{itemize}
    \item 对于表达式$R = R_1 | R_2 | \cdots | R_n$，且$\forall i \text{ 满足 } 1 \le i \le n, \forall R_i \ne \varepsilon$，有$q_{start}^{R_i} \in q_{start}^{R}$
    \item 对于表达式$R = R_1 | R_2 | \cdots | R_n$，且$\forall i \text{ 满足 } 1 \le i \le n, \forall R_i \ne \varepsilon$，有$q_{end}^{R_i} \in q_{end}^{R}$
    \item 对于表达式$R = P | \varepsilon | S$，或者表达式$R = \varepsilon | S$，或者表达式$R = P | \varepsilon$，有
    ...
    \item 对于表达式$R = R_1 | R_2 | \cdots | R_n$，如果这个或运算的作用域为表达式的顶层，设$q_0$为NFA的起始状态，则$\forall q_s \in q_{start}^{R}, \exists \delta(q_0, c(q_s)) = q_s$
\end{itemize}
...
\qquad 对于闭包运算$R_0*$，定义以下构建规则：

闭包运算的转移函数定义：
\begin{itemize}
    \item $(\varepsilon)* = \varepsilon$，下文提及的$R_0 \ne \varepsilon$.
    \item 对于$R = R_0*$，$q_{start}^{R} = q_{start}^{R_0}$，$q_{end}^{R} = q_{end}^{R_0}$.
...
\qquad 对于反运算$\sim R_0$，定义以下构建规则：
\begin{itemize}
    \item $\sim (\sim R_0) = \sim \sim R_0 = R_0$.
    \item 对于$R_0 = R_1 \cdot R_2, l = |R_2|, \sim R_0 = \sim R_1 \bullet^l | R_1 \sim R_2$.
    \item 对于$R_0 = R*, l = |R|, \sim R_0 = \sim R (\bullet^l)* | R \sim R_0$.
    \item 对于$R_0 = R_1 | R_2 | \cdots | R_n, \sim R_0 = \mathrm{ILU}(\sim R_1, \sim R_2, \cdots, \sim R_n)$. \\
          $\mathrm{ILU}$是一个对参数$\sim R_1, \sim R_2, \cdots, \sim R_n$取交集，但在长度上取并集的处理函数，这里不做详细解释，将在后文说明。
\end{itemize}

\subsection{具体实现}

\qquad 接下来将讨论在计算机上，构建规则的具体实现细节。

\subsubsection{状态节点的结构}

\[\text{定义}: \var{CHARACTER}\text{为字母表中所有字符的种类数量}\]
\[\text{定义}: \text{以}\var{TYPEVALUE}\text{为状态数的数胞在结构体中后继以}\var{CHARACTER}\text{为状态数的数胞刚好}\]
\[\text{填充完以}\var{WORDSIZEVALUE}\text{为状态数的数胞所占的全部存储空间，且}\var{TYPEVALUE} \ge 4\]

\[\begin{bmatrix}
    \text{结构体}: MST \\
    character \in NC_{\var{CHARACTER}} \\
    type \in NC_{\var{TYPEVALUE}}
\end{bmatrix}\]

\[\begin{bmatrix}
    \text{联合体}: ST \\
    index \in SNC_{\var{WORDSIZEVALUE}} \\
    mst \in MST
\end{bmatrix}\]

其中，$MST$代表一个有实际意义的节点，它有确定的转移条件$character$和类型$type$。
在实际实现中，我们通过$type$的取值限定来区分一个状态节点$ST$是$MST$还是一个索引$index$，但在这里没法表现区分的方法。
区分的方法为：如果$ST$实例中$index$是一个负数，那么这个$ST$就是一个$MST$，否则是一个索引。也就是说，$type$一旦取值，$index$就必须是负数。
至于为什么能这么实现，因为计算机的地址的状态数为$\var{WORDSIZEVALUE}$，而$ST$满足不只占一个字节的存储空间，因此，就算计算机中的存储空间全部用于存储$ST$，数量也不会超过$\var{WORDSIZEVALUE}$的一半，
...
但是，如果只有$MST$，我们无法描述NFA中一个$MST$到另一个$MST$的转移路径（使用状态节点的线性数组表示NFA）。
因此，我们需要额外的一种类型——转移节点，用于表示不在NFA中直接相邻的两个$MST$之间的转移路径（默认两个$MST$在NFA中相邻，即之间没有任何其他节点，那么它们之间存在转移路径）。
我们规定，一个$MST$后面如果紧接一个或一连串索引节点，那么这些索引节点都修饰这个$MST$。
综上所述，我们使用$MST$表示实际的状态，用索引节点的索引表示它所修饰的实际状态拥有哪些转移路径（索引的含义为NFA中$MST$的索引）。

\subsubsection{虚转移节点}

\qquad 由于在构建过程中，有时难以判断一个$MST$之后需要多少个索引节点，而NFA又是使用数组表示，导致难以确定节点的位置安放和索引节点的索引该如何选取。
...
在这个展开过程完成之后，我们认为NFA中的所有索引节点都是转移节点，即不存在虚转移节点了。
