...
\begin{itemize}
    ...
    \item 对于表达式$R = (R_0)* | \cdots$，且$Q_f^{or}$为$R$所在的或运算的所有后继状态的集合，则$q_{start}^{R} \in \{q_{start}^{R_0}\} \cup Q_f^{or}$
    \item 对于表达式$R = \cdots | (R_0)*$，且$Q^{or}$为$R$所在的或运算的所有前驱状态的集合，则$q_{end}^{R} \in \{q_{end}^{R_0}\} \cup Q^{or}$
